...
function ChatPageContent() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const res = await fetch(`/api/chats/chat-sessions?wsId=${activeWsId}`);
        if (!res.ok) throw new Error('API Error');
        const data = await res.json();
        
        // 🔥 กรองเอาเฉพาะ FACEBOOK
        const fbChats = Array.isArray(data) ? data.filter(c => c.platform === 'FACEBOOK') : [];
        setChats(fbChats);
        ...
        if (tagRes.ok) {
          ...
          console.error('❌ Failed to load Facebook chats:', error);
          setChats([]);
      } finally {
          setIsLoaded(true);
      }
  ...
  const reloadChats = async () => {
      try {
        ...
        if (chatRes.ok) {
          ...
          const fbChats = Array.isArray(chatData) ? chatData.filter(c => c.platform === 'FACEBOOK') : [];
          setChats(fbChats);
        }
      } catch (error) {
  ...
  // 🔥 2. ดักฟัง Pusher (Real-time ล้วนๆ)
  useEffect(() => {
    if (!isLoaded) return;
    ...
    const setupPusher = async () => {
        ...
        channel.bind('webhook-event', function(data) {
          ...
          if (data.platform && data.platform !== "FACEBOOK") return; // กรองเฉพาะ FACEBOOK
  ...
  const addLog = (chatId, type, detail) => {
    if (!chatId) return;
    const newLog = { id: Date.now() + Math.random(), chatId, type, detail, timestamp: new Date().toISOString(), by: currentUser.name };
  ...
  const handleAddNote = async (noteData) => {
    ...
    setChats((currentChats) =>
      currentChats.map((chat) => chat.id === selectedChatId ? { ...chat, notes: [...(chat.notes || []), noteData] } : chat)
    );
  };
    ...
    return (
      <div className="flex-1 flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-[#BE7EC7] text-sm font-medium">Loading Facebook Chats...</div>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 flex gap-5 px-7 py-3 overflow-hidden">
        <ChatList
          ...
          onSelectChat={async (chat) => {
            ...
            try {
              await fetch(`/api/chats/${chat.id}/read`, { method: "PATCH" });
            } catch (err) {
        ...
        <div className="flex-1 flex gap-5 overflow-hidden relative">
          ...
          {isAddTagModalOpen && (
            <AddTag onClose={() => setIsAddTagModalOpen(false)} availableTags={availableTags} currentTargets={selectedChat?.tags || []} onToggleTag={handleToggleTag} />
          )}
          {isContactDetailsOpen && (
            <ContactDetails onClose={() => setIsContactDetailsOpen(false)} contact={selectedChat} onUpdateContact={handleUpdateContactInfo} />
          )}
          {isAddNoteOpen && (
            <AddNote onClose={() => setIsAddNoteOpen(false)} onSaveNote={handleAddNote} currentNotes={selectedChat?.notes || []} onDeleteNote={(id) => { setChats((prev) => prev.map((c) => c.id === selectedChatId ? { ...c, notes: c.notes.filter((n) => n.id !== id) } : c)); }} />
          )}
          {isChangeStatusOpen && (
            <ChangeStatus onClose={() => setIsChangeStatusOpen(false)} availableStatus={ALL_AVAILABLE_STATUS} currentTargets={selectedChat?.status ? [selectedChat.status] : []} onToggleStatus={handleUpdateStatus} />
          )}
          {isActivityLogOpen && (
            <ActivityLogPanel onClose={() => setIsActivityLogOpen(false)} logs={activityLogs.filter((log) => log.chatId === selectedChatId)} />
          )}
          ...
          {isAiAssistantOpen && (
            <AiAssistantPanel onClose={() => setIsAiAssistantOpen(false)} availableAgents={availableAgents} />
          )}
        </div>
      </div>
