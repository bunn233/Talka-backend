...
function ChatPageContent() {
    ...
    // 🟢 Fetch Initial Data (ดึงเฉพาะ INSTAGRAM และตรงกับ Workspace ปัจจุบัน)
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                // 🔥 3. กรองเอาเฉพาะ INSTAGRAM
                const igChats = Array.isArray(data) 
                    ? data.filter(c => c.platform === 'INSTAGRAM') 
                    : [];
                ...
                console.error('❌ Failed to load Instagram chats:', error);
                setChats([]);
            } finally {
                setIsLoaded(true);
            }
    ...
    const reloadChats = async () => {
        try {
            ...
            if (chatRes.ok) {
                ...
                const igChats = Array.isArray(chatData) ? chatData.filter(c => c.platform === 'INSTAGRAM') : [];
                setChats(igChats);
            }
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const setupPusher = async () => {
            ...
            channel.bind('webhook-event', function(data) {
                ...
                if (data.platform && data.platform !== "INSTAGRAM") return; // กรองเฉพาะ INSTAGRAM
        ...
        return (
            <div className="flex-1 flex h-full items-center justify-center">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="text-[#BE7EC7] text-sm font-medium">Loading Instagram Chats...</div>
                </div>
            </div>
        );
    }
